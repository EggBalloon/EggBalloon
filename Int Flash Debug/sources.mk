################################################################################
# Automatically-generated file. Do not edit!
################################################################################

SX_SRCS := 
O_SRCS := 
ASM_UPPER_SRCS_QUOTED := 
OBJ_SRCS_OS_FORMAT := 
TODISASSEMBLE_SRCS_OS_FORMAT := 
S_UPPER_SRCS_QUOTED := 
S_UPPER_SRCS := 
SX_SRCS_QUOTED := 
TODISASSEMBLE_SRCS := 
OBJ_SRCS_QUOTED := 
O_SRCS_OS_FORMAT := 
TOPREPROCESS_SRCS := 
C_SRCS_QUOTED := 
S_SRCS_OS_FORMAT := 
SX_SRCS_OS_FORMAT := 
O_SRCS_QUOTED := 
OBJ_SRCS := 
ASM_SRCS := 
TOPREPROCESS_SRCS_OS_FORMAT := 
ASM_UPPER_SRCS := 
ASM_SRCS_OS_FORMAT := 
C_SRCS := 
S_SRCS := 
ASM_SRCS_QUOTED := 
S_SRCS_QUOTED := 
TOPREPROCESS_SRCS_QUOTED := 
ASM_UPPER_SRCS_OS_FORMAT := 
C_SRCS_OS_FORMAT := 
TODISASSEMBLE_SRCS_QUOTED := 
S_UPPER_SRCS_OS_FORMAT := 
SX_DEPS_OS_FORMAT := 
S_DEPS_QUOTED := 
ASM_DEPS_OS_FORMAT := 
OBJS := 
C_DEPS := 
SX_DEPS := 
S_UPPER_DEPS_OS_FORMAT := 
ASM_DEPS_QUOTED := 
EXECUTABLES_OS_FORMAT := 
S_DEPS := 
S_DEPS_OS_FORMAT := 
S_UPPER_DEPS := 
ASM_DEPS := 
ASM_UPPER_DEPS_QUOTED := 
ASM_UPPER_DEPS := 
SX_DEPS_QUOTED := 
OBJS_QUOTED := 
C_DEPS_OS_FORMAT := 
ASM_UPPER_DEPS_OS_FORMAT := 
EXECUTABLES_QUOTED := 
S_UPPER_DEPS_QUOTED := 
EXECUTABLES := 
C_DEPS_QUOTED := 
OBJS_OS_FORMAT := 

# Every subdirectory with source files must be described here
SUBDIRS := \
services/OS \
services/Com_Manager \
mcal/IO_drivers/Pwm \
mcal/IO_drivers/Dio \
mcal/IO_drivers/Adc \
mcal/COM_Drivers/Spi \
ecu/IO_HW_abs/IO_Interface \
ecu/Com_HW_abs/Com_Interface \
Source \
Complex_Drivers/Sdio \
Complex_Drivers/Sci \
Application/Swc_Temp \
Application/Swc_Sd \
Application/Swc_Motor \
Application/Swc_Led \
Application/Swc_Gps \

